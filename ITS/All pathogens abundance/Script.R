...
#### 5. ALL PATHOGENS VS OTHER ####

merge_ft <- merge(tax, DATABASE, by = "Genus", all.x = TRUE)
merge_ft <- merge_ft[match(rownames(ASV), merge_ft$ASV), ]

guild <- ifelse(
  grepl("patho", merge_ft[[lifestyle_col]], ignore.case = TRUE),
  "All_pathogens",
  "Other"
)

guild_levels <- c("All_pathogens", "Other")
...
for(g in guild_levels){
  idx <- which(guild == g)
  if(length(idx) > 0){
    guild_mat[g, ] <- colSums(ASV[idx, , drop = FALSE])
  }
}
...
run_type_pathogens <- function(type_name){
  ...
  plotQQunif(
    sim_res,
    main = paste(type_name, "- All pathogens")
  )
  ...
  p <- ggplot(d, aes(x = Landuse, y = CLR, fill = Landuse)) +
    ...
    labs(
      title = paste(type_name, "All pathogens abundance"),
      x = NULL,
      y = "CLR abundance"
    )
...
res_leaf_pathogens <- run_type_pathogens("Leaf")
res_root_pathogens <- run_type_pathogens("Root")
res_soil_pathogens <- run_type_pathogens("Soil")

res_leaf_pathogens$plot
res_root_pathogens$plot
res_soil_pathogens$plot
