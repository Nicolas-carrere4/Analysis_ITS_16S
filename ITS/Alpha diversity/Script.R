...
ps_ITS_raw <- readRDS(paste0(path, "phyloseq_no_negatives_ITS.rds"))
...
get_shannon_ITS <- function(type_name){

  ps_sub <- get_ps_by_type(ps_ITS_raw, type_name)
...
plot_shannon_kruskal <- function(type_name){

  d <- get_shannon_ITS(type_name)
  ...
  p <- ggplot(d, aes(x = Landuse, y = Shannon, fill = Landuse)) +
    ...
    labs(
      title = paste("ITS Shannon diversity -", type_name),
      x = NULL,
      y = "Shannon diversity"
    )
...
p_leaf_ITS_shannon <- plot_shannon_kruskal("Leaf")
p_root_ITS_shannon <- plot_shannon_kruskal("Root")
p_soil_ITS_shannon <- plot_shannon_kruskal("Soil")
