...
run_homogenization_lmm_16S <- function(type_name){
  ...
  samples <- intersect(samples, labels(dist_16S))
  ...
  dist_sub <- as.dist(as.matrix(dist_16S)[samples, samples])
  ...
  p <- ggplot(
    ...
    labs(
      title = paste("Homogenization LMM - 16S -", type_name),
      x = "Land use",
...
hom_leaf_16S <- run_homogenization_lmm_16S("Leaf")
hom_root_16S <- run_homogenization_lmm_16S("Root")
hom_soil_16S <- run_homogenization_lmm_16S("Soil")
